export const KIMI_VOICE_ROUTER_PROMPT_VERSION =
  'kimi-voice-command-router-v1.3.0';
...
2. 一句话包含多个要求时，选择最主要的 intent；可同时通过 playbackRate 表示“慢一点/快一点”。
   playbackRate 只允许 0.5、0.75、1、1.25、1.5；“正常速度/原速”是 1。
...
5. 与舞蹈教学和练习完全无关时，intent 必须为 null；responseText 仍要自然回应用户，提供简短、积极的情绪价值，再温和引导回练习，不要重复固定模板。
