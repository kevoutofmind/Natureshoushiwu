...
const viewerFile = resolve(
  projectRoot,
  'bknd',
  'tools',
  'skeleton-qc-viewer.html',
);
const viewerScript = resolve(
  projectRoot,
  'bknd',
  'tools',
  'skeleton-qc-viewer.js',
);
...
function safeChild(root, encodedPath) {
  const fullPath = resolve(root, decodeURIComponent(encodedPath));
    ...
    .flatMap((entry) => {
      try {
        const dataset = JSON.parse(
          readFileSync(resolve(dataRoot, entry.name), 'utf8'),
        );
        if (!Array.isArray(dataset.videos)) return [];
        const datasetId =
          dataset.datasetId || entry.name.replace(/\.json$/i, '');
        return [
          {
            file: entry.name,
            datasetId,
            title: dataset.title || datasetId || entry.name,
            ...
            videos: dataset.videos.map((video) => {
              const filename = basename(video.sourceFile);
              const rootPath = filename;
              const categoryPath = `${datasetId}/${filename}`;
              const videoPath = existsSync(resolve(videoRoot, rootPath))
                ? rootPath
                : existsSync(resolve(videoRoot, categoryPath))
                  ? categoryPath
                  : null;
              return {
...
function extractionProgress() {
  const groups = monitoredGroups.map((groupName) => {
    ...
    try {
      ...
      const explicitSkipped = skipped.length;
      const successful = Array.isArray(dataset.videos)
  ...
  const recentRelevant = logLines
    .filter((line) =>
      /(?:VIDEO |HEARTBEAT |TRANSCODE|SOURCE_READY|SAVED |GAVE_UP |COMPLETE |WATCHDOG_|RETRY )/.test(
        line,
      ),
    )
    .slice(-12);
...
function streamFile(request, response, fullPath) {
  ...
  const contentType =
    mimeTypes[extname(fullPath).toLowerCase()] || 'application/octet-stream';
...
const server = createServer((request, response) => {
  try {
    ...
    if (url.pathname === '/viewer.js') {
      streamFile(request, response, viewerScript);
      return;
    }
    if (url.pathname === '/vision_bundle.mjs') {
      streamFile(request, response, visionBundle);
      return;
    }
    if (url.pathname.startsWith('/mediapipe/')) {
      streamFile(
        request,
        response,
        safeChild(mediaPipeRoot, url.pathname.slice('/mediapipe/'.length)),
      );
      return;
    }
    ...
    if (url.pathname.startsWith('/video/')) {
      streamFile(
        request,
        response,
        safeChild(videoRoot, url.pathname.slice(7)),
      );
      return;
    }
